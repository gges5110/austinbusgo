...
export const GtfsRtFrontendDevPage: React.FC = () => {
  ...
  } = useGtfsRtTripUpdates(routeFilter ? { routeId: routeFilter } : undefined);
          ...
          >
            <Box>
              ...
              <Typography color={"text.secondary"} variant={"caption"}>
                {tripUpdatesFetching
                  ? "Fetching…"
                  : `${(tripUpdates ?? []).length} trip update${(tripUpdates ?? []).length !== 1 ? "s" : ""}${routeFilter ? ` for route ${routeFilter}` : ""} · updated ${tripUpdatesUpdatedAt ? dayjs(tripUpdatesUpdatedAt).fromNow() : "—"}`}
              </Typography>
            </Box>
